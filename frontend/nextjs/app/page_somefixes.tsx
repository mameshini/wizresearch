...
export default function Home() {
  ...
  // Initialize currentApiUrl from localStorage or environment variables
  useEffect(() => {
    const storedConfig = localStorage.getItem('apiVariables');
    const apiVariables = storedConfig ? JSON.parse(storedConfig) : {};
    const apiUrl = apiVariables.GPTR_API_URL || process.env.NEXT_PUBLIC_GPTR_API_URL || '';
    setCurrentApiUrl(apiUrl);
  }, []);

  const { socket, initializeWebSocket, isConnected } = useWebSocket(
    setOrderedData,
    setAnswer,
    setLoading,
  ...
  const handleDisplayResult = async (newQuestion: string) => {
    ...
    setPromptValue(newQuestion); // Set promptValue to newQuestion for proper WebSocket initialization
    
    // Add question to ordered data
    ...
    if (chatBoxSettings.report_type === 'multi_agents' && langgraphHostUrl) {
      ...
      // Initialize new WebSocket with the new question
